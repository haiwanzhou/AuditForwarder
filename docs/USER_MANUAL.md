...
服务端内置跨操作员合谋协同检测子系统（v1.1.0 新增），对多操作员在短时间内协同完成的高风险操作链进行自动识别：

- 双窗口聚合：短窗口（默认 15 分钟）定性、长窗口（默认 4 小时）弱关联线索。
- 规则引擎：内置 5 条典型合谋规则（备份销毁、密钥权限拆分绕过、配置篡改、数据批量外泄、权限晋升），规则可配置、热加载、可扩展。
- 行为画像：基于操作员历史协作关系与资产组访问基线，对陌生协作、跨域操作给出线索评分。
- 工单核验：关联工单须通过外部注册表四要素核验（真实存在、状态有效、人员匹配、有效期覆盖），伪造工单加重评分。
- 风险分级：HIGH（≥80 分）/ MID（40~79 分）/ LOW（1~39 分），中高风险自动推送告警。
- 处置闭环：支持「待复核 / 确认合谋攻击 / 合法运维（误报）/ 待进一步调查」四态处置，处置全程留审计日志。
- 应急运维模式：开启后风险等级自动降一级，适配变更窗口等场景。
- 该模块为并行无侵入扩展：不修改原有审计链路、不自动阻断业务。

检测数据保存在 `data/server/collusion/`，配置与规则保存在 `data/server/policies/collusion_*.json`（首次启动自动生成默认值）。
...
- `auth_token`：固定管理 Token，保留给调试和脚本调用。
- `login_username`：管理页面登录账号。
- `login_password_sha256`：管理页面登录密码的 SHA-256 哈希。
- `enrollment_key`：客户端加入监管系统时使用的注册密钥。
...
- 顶部可按采集器类型（如 `etw_win`、`file_win`）和优先级（high / normal）过滤。
- 统计卡片展示日志总数、告警总数、操作类型分布和告警级别分布。
- 两个列表分别展示最新违规告警和最新结构化操作日志。
- 合谋检测产生的中高风险事件也会以 `source=collusion` 出现在告警列表中。

### 7.10 日志策略

「日志策略」页面用于按采集器配置过滤档位（strict / lenient）：

- 消息最大长度、丢弃字段、留存行数均可调，保存后热生效。
- ETW 安全日志默认走宽松档，保留完整原始字段；其他采集器默认严格档。
- 可按主机 ID 查询实际生效的策略档。

### 7.11 合谋检测

左侧导航点击「合谋检测」进入跨操作员合谋协同检测页面：

1. 顶部四张卡片展示高 / 中 / 低风险事件数量与待复核数量。
2. 「引擎状态」区展示双窗口时长、风险阈值、规则与资产组数量、画像覆盖人数与协作对数，可一键开启/解除应急运维模式（默认 120 分钟）。
3. 「风险事件」区支持按级别和处置状态过滤；每条事件展示评分、命中规则、涉事操作员、判定依据，可执行：
   - **查看详情**：在下方报表区输出完整事件 JSON（含证据时间线）。
   - **导出证据链**：下载 `<event_id>.json` 完整证据文件。
   - **处置**：选择处置状态并填写说明，提交后立即生效并写入合谋检测审计日志。
4. 「分析报表」区输入天数（1~180）生成统计报表，含各级别事件数、规则命中分布、确认攻击数与误报数。
5. 底部三个编辑区分别维护检测配置、规则库和工单注册表，保存后热生效，无需重启服务端。
...
```text
GET  /collusion/overview             检测引擎概览（事件计数/窗口/画像统计）
GET  /collusion/events               风险事件列表（参数 level、status、limit）
GET  /collusion/events/detail?id=    单个风险事件完整详情（含证据时间线）
GET  /collusion/events/export?id=    导出证据链 JSON
POST /collusion/events/dispose       处置风险事件（event_id/status/disposer/note）
GET  /collusion/config               读取检测配置
PUT  /collusion/config               保存检测配置（热生效）
GET  /collusion/rules                读取规则库
PUT  /collusion/rules                保存规则库（热生效）
GET  /collusion/workorders           读取工单注册表
PUT  /collusion/workorders           保存工单注册表
POST /collusion/emergency            开启/解除应急运维模式（enabled/duration_minutes）
GET  /collusion/report?days=7        生成近 N 天合谋行为分析报表
POST /collusion/ingest               原生格式事件接入（演练与外部系统对接）
```
...
合谋检测只在窗口到期后才输出评估结果，属正常设计：

- 短窗口（默认 15 分钟）结束并经过宽限期（默认 10 秒）后才冲刷评估；注入演练事件时把 `event_timestamp` 设为已结束窗口内的时间，或等待当前窗口到期。
- 集合内不同操作员少于 2 人的事件会被直接丢弃（单人操作交由原有单条审计处理）。
- 执行失败的操作（`return_code` 非 0 或 `outcome=failed`）不参与检测。
- 资源键（`res_key`/`target`）必须能命中配置中的某个 `asset_groups` 通配符，否则不归入任何聚合组。
- 检查检测配置是否 `enabled: true`；白名单协同组的事件只保留 0 分低风险线索。
- 中高风险事件同时写入 `data/server/alerts/<host>.jsonl`，可在「日志与告警」页面交叉确认。

### 11.8 合谋配置保存后未生效

检测配置与规则库每 2 秒检查一次文件变更并热加载，无需重启。若未生效：

- 确认 JSON 语法正确（非法字段会回退为默认值，窗口时长越界会被自动钳制：短窗 5~30 分钟、长窗 1~24 小时）。
- `asset_groups` 不能为空，至少保留一个资产组。
- 规则必须含 `rule_id` 和非空 `op_sequence`，`enabled` 为 true 才参与检测。
- 所有配置变更与处置操作都会写入 `data/server/collusion/audit.jsonl`，可据此核对。
...
- 新增跨操作员合谋协同检测子系统：双窗口聚合、R-001~R-005 规则引擎、行为画像评分、工单外部核验、风险分级与告警降噪。
- 新增风险事件四态处置闭环（待复核 / 确认合谋攻击 / 合法运维（误报）/ 待进一步调查）、证据链导出与分析报表。
- 新增应急运维模式（风险等级自动降一级）和检测配置/规则/工单热加载。
- 新增 `/collusion/*` REST API 与 Web「合谋检测」页面。
- 新增客户端-服务端协同日志分类与优先级管理（采集器标识、提权标注、双队列传输、按主机/采集器隔离存储）。
- 新增 ETW 安全日志宽松过滤档与可配置日志策略管理页面。
- 新增 Web 控制台一键生成客户端接入包、主机接入与采集器控制页面。
- 单元测试扩展至 175 项，新增合谋检测纯逻辑/工单核验/评分/引擎端到端四组测试。
...
9. 查看命令结果和审计摘要
10. 打开「合谋检测」页面，确认引擎运行中；可通过 /collusion/ingest 接口注入演练事件验证告警与处置闭环
11. 根据日志、批次文件和登录审计记录进行审计分析
```
